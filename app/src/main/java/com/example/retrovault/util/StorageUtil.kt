...
object StorageUtil {
    ...
    fun saveImageToRetroVaultDir(context: Context, sourceUri: Uri): Uri? {
        return try {
    ...
    fun saveBitmapToRetroVaultDir(context: Context, bitmap: android.graphics.Bitmap, oldUriString: String? = null): Uri? {
        return try {
            val fileName = "cover_${UUID.randomUUID()}.webp"
    ...
    suspend fun createBackupZip(context: Context, backupData: com.example.retrovault.data.BackupData, includeImages: Boolean): ByteArray? = withContext(Dispatchers.IO) {
        try {
            val outputStream = java.io.ByteArrayOutputStream()
            ...
            fun backupImages(items: List<Any>): List<Any> {
                ...
                return items.map { item ->
                    val coverUri = when (item) {
                        is com.example.retrovault.data.Game -> item.coverUri
                        is com.example.retrovault.data.Movie -> item.coverUri
                        is com.example.retrovault.data.Music -> item.coverUri
                        else -> ""
                    }
                        ...
                        if (fileName != null) {
                            try {
                                ...
                                return@map when (item) {
                                    is com.example.retrovault.data.Game -> item.copy(coverUri = fileName)
                                    is com.example.retrovault.data.Movie -> item.copy(coverUri = fileName)
                                    is com.example.retrovault.data.Music -> item.copy(coverUri = fileName)
                                    else -> item
                                }
                            } catch (e: Exception) {
                                e.printStackTrace()
            ...
            val gamesForBackup = backupImages(backupData.games) as List<com.example.retrovault.data.Game>
            @Suppress("UNCHECKED_CAST")
            val moviesForBackup = backupImages(backupData.movies) as List<com.example.retrovault.data.Movie>
            @Suppress("UNCHECKED_CAST")
            val musicForBackup = backupImages(backupData.music) as List<com.example.retrovault.data.Music>
            ...
            // Add JSON file
            val backupJsonString = json.encodeToString(com.example.retrovault.data.BackupData.serializer(), finalBackupData)
    ...
    suspend fun restoreFromBackupZip(context: Context, uri: Uri): String? = withContext(Dispatchers.IO) {
        try {
            ...
            val backupData = try {
                json.decodeFromString(com.example.retrovault.data.BackupData.serializer(), backupJson!!)
            } catch (e: Exception) {
                // Legacy fallback
                val games = json.decodeFromString<List<com.example.retrovault.data.Game>>(backupJson!!)
            ...
            fun updateUris(items: List<Any>): List<Any> {
                return items.map { item ->
                    ...
                    if (tempImages.containsKey(coverUri)) {
                        ...
                        return@map when (item) {
                            is com.example.retrovault.data.Game -> item.copy(coverUri = newUri)
                            is com.example.retrovault.data.Movie -> item.copy(coverUri = newUri)
                            is com.example.retrovault.data.Music -> item.copy(coverUri = newUri)
                            else -> item
                        }
                    }
                    item
            ...
            @Suppress("UNCHECKED_CAST")
            val finalGames = updateUris(backupData.games) as List<com.example.retrovault.data.Game>
            @Suppress("UNCHECKED_CAST")
            val finalMovies = updateUris(backupData.movies) as List<com.example.retrovault.data.Movie>
            @Suppress("UNCHECKED_CAST")
            val finalMusic = updateUris(backupData.music) as List<com.example.retrovault.data.Music>
            ...
            json.encodeToString(com.example.retrovault.data.BackupData.serializer(), finalData)
        } catch (e: Exception) {
            e.printStackTrace()
            null
        }
    ...
    private fun saveByteArray(context: Context, data: ByteArray, fileName: String): Uri? {
        return saveToRetroVaultStorage(context, fileName) { outputStream ->
    ...
    private fun saveToRetroVaultStorage(context: Context, fileName: String, writeBlock: (java.io.OutputStream) -> Unit): Uri? {
        return try {
